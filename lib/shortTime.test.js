...
test("Invalid time (Noon)", () => {
  expect(() => parseShortTime("Noon")).toThrowError(Error("Invalid time format"));
});

test("Invalid time (13:00 AM)", () => {
  expect(() => parseShortTime("13:00 AM")).toThrowError(Error("Invalid time format"));
});

test("Invalid time (69)", () => {
  expect(() => parseShortTime("69")).toThrowError(Error("Invalid time format"));
});
