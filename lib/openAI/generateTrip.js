...
export async function generateTrip(body) {
  logger.info("Generating tripJSON from ChatGPT");
  ...
  while (chatAttemps < CHATGPT_RETRIES) {
    // Increment
    chatAttemps += 1;
    logger.info(`Query attempt #${chatAttemps}`);
    ...
    try {
      ...
      logger.info(`Received back a response with length ${rawJSON.length}`);

      const tokens = response.usage.total_tokens;
      logger.info(`Used ${tokens} tokens`);
    } catch (err) {
      logger.error("Failed to query ChatGPT", err);
      continue;
    }
    ...
    try {
      ...
      logger.error("Failed to validate JSON", err);
      failMessage = err;
      continue;
    }
  }
  logger.error(`Unable to generate trip after ${CHATGPT_RETRIES} attempts`);
