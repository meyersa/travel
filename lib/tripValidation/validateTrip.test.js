...
test("No description", async () => {
  ...
  await expect(validateTrip(badJSON)).rejects.toThrowError(
...
test("Text over 300 chars", async () => {
  ...
  await expect(validateTrip(badJSON)).rejects.toThrowError(
    Error("Text field over max char amount")
  );
});
...
test("Text field under 5 chars", async () => {
  ...
  await expect(validateTrip(badJSON)).rejects.toThrowError(
    Error("Text field under max char amount")
  );
});
...
test("Invalid number of days compared to Start/End", async () => {
  ...
  await expect(validateTrip(badJSON)).rejects.toThrowError(
    Error("Trip must include the same amount of days as dates")
  );
});
...
test("Days are not sequential", async () => {
  ...
  await expect(validateTrip(badJSON)).rejects.toThrowError(
    Error("Days must be in sequential order")
  );
});
...
test("Stop has invalid start time", async () => {
  ...
  await expect(validateTrip(badJSON)).rejects.toThrowError(Error("Invalid time format, more than 4 matches hi"));
});
...
test("Stop has invalid stop time", async () => {
  ...
  await expect(validateTrip(badJSON)).rejects.toThrowError(Error("Invalid time format, more than 4 matches 2016-5"));
});
...
test("Overlapping stop times", async () => {
  ...
  await expect(validateTrip(badJSON)).rejects.toThrowError(
    Error("Stop time conflicts with previous stop")
  );
});
...
test("Additional has no points", async () => {
  ...
  await expect(validateTrip(badJSON)).rejects.toThrowError(
    Error("Additional parameters are not valid")
  );
});
