...
export async function createBackgroundImages(id) {
  ...
  for (let day of tripJSON.days) {
    for (let stop of day.stops) {
      try {
        ...
        if (!stop.pictureSearchTerms || stop.pictureSearchTerms.trim() === "" || stop.pictureSearchTerms.trim() === "default") continue;
  ...
  logger.info(`Finished. Generated ${count} images for trip stops`);
