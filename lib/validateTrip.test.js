...
const goodJSON = {
  ...
  pictureUrl: "https://cdn.britannica.com/13/77413-050-95217C0B/Golden-Gate-Bridge-San-Francisco.jpg",
  days: [
    {
      name: "Day 1: Arrival and Golden Gate",
      date: "2025-04-16",
      description: "Arrive in San Francisco and visit the world-famous Golden Gate Bridge.",
      pictureUrl: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSYWxU23Zn1ZvuXze1EU5MTp23EAvonnn-ckw&s",
      stops: [
        {
          ...
          pictureUrl: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTzsa39Ct_wlG-0JAPqMIuZOyiJisvdO_Df_Q&s",
        },
      ],
    },
    {
      ...
      stops: [
        {
          ...
          pictureUrl: "https://media.cntraveler.com/photos/5fffccfcd273ea22962091eb/16:9/w_2560,c_limit/1225606936",
        },
        {
          name: "Lombard Street",
          startTime: "2pm",
...
test("Text over 300 chars", async () => {
  let badJSON = JSON.parse(JSON.stringify(goodJSON));
  badJSON.description = "A".repeat(301);
...
test("Text fields at exactly 300 chars", async () => {
  let badJSON = JSON.parse(JSON.stringify(goodJSON));
  badJSON.description = "A".repeat(300);
...
test("Stop has invalid lat/long", async () => {
  let badJSON = JSON.parse(JSON.stringify(goodJSON));
  badJSON.days[0].stops[0].lattitude = "invalid";
