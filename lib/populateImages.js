...
export async function getImage(GOOGLE_CONSOLE_ID, GOOGLE_API_KEY, query) {
  ...
  var res = await axios.get(
    `https://www.googleapis.com/customsearch/v1?key=${GOOGLE_API_KEY}&cx=${GOOGLE_CONSOLE_ID}&q=${query}`
  );
  ...
  for (let imgRaw of res) {
    ...
    if (img) {
      ...
      // return { img: img, source: source, title: title };
...
export async function populateImages(GOOGLE_CONSOLE_ID, GOOGLE_API_KEY, tripJSON) {
  ...
  tripJSON["pictureUrl"] = await getImage(GOOGLE_CONSOLE_ID, GOOGLE_API_KEY, tripJSON["pictureSearchTerms"]);
  ...
  for (let day of tripJSON["days"]) {
    for (let stop of day.stops) {
      try {
        ...
        stop["pictureUrl"] = await getImage(GOOGLE_CONSOLE_ID, GOOGLE_API_KEY, stop["pictureSearchTerms"]);
      } catch (e) {
        continue;
      }
    }
  ...
  for (let add of tripJSON["additional"]) {
    try {
      ...
      add["pictureUrl"] = await getImage(GOOGLE_CONSOLE_ID, GOOGLE_API_KEY, add["pictureSearchTerms"]);
    } catch (e) {
      continue;
    }
  }
